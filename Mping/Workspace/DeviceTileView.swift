...
struct DeviceTileView: View {
    ...
    private var pingBadge: some View {
        Text(tilePingText)
            ...
            .font(.system(size: tilePingText.contains("\n") ? max(7, settings.pingValueSize * 0.78) : settings.pingValueSize, weight: settings.pingValueBold ? .bold : .semibold, design: .monospaced))

            .italicIf(settings.pingValueItalic)

            .foregroundStyle(.white.opacity(settings.pingValueOpacity))

            .lineLimit(tilePingText.contains("\n") ? 2 : 1)
    ...
    /// The device tile must never show "Timeout".
    ///
    /// Online or verifying:
    /// - keep showing the most recent valid RTT.
    ///
    /// Confirmed offline:
    /// - show Last Seen.
    ///
    /// This makes the tile independent from transient ping failures while the
    /// verification engine is doing its job.
    private var tilePingText: String {
        if shouldShowLastSeen {
            return lastSeenDisplayText
        }
        ...
        if let lastSeen = device.lastSeenOnline {
            return "Seen " + Self.lastSeenFormatter.string(from: lastSeen)
        }
    ...
    private var shouldShowLastSeen: Bool {
        ...
        // Defensive fallback: if the verification state is already offline but the visible
        // status has not caught up yet, prefer Last Seen over any transient ping failure text.
        if device.verificationState == .offline {
            return true
        }
    ...
    private var latestValidRTT: Double? {
        if let rtt = device.lastRTT, rtt.isFinite, rtt >= 0 {
            return rtt
        }
